...
class CNN(Model) :
    def __init__(model, nb_classes, in_shape=None):
        ...
        model.build_model()
        Model.__init__(model, model.x, model.y)
        model.compile()
...
class DataSet :
    def __init__(self, X, y, nb_classes, scaling=True, test_size=0.2, random_state=0):
        ...
        if scaling :
            scalar = MinMaxScaler()
            n = X_train.shape[0]
            X_train = scalar.fit_transform(X_train.reshape(n,-1)).reshape(X_train.shape)
            n = X_test.shape[0]
            X_test = scalar.transform(X_test.reshape(n , -1)).reshape(X_test.shape)
...
class Machine() :
    ...
    def run(self, epochs=1, batch_size=128, verbose=1):
        data = self.data
        model = self.model
        fig = self.fig
